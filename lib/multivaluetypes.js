...
/**
  Convenient helper function to automatically return MultiValueType
*/
module.exports.words = function(Type, words) {
  var firstWord = words.shift()
    , type = new Type(firstWord);
  for(var i = 0, len = words.length; i < len; ++i) {
    if(type.isFinished()) return type.valueOf();
    type.give(words[i]);
  }
  return type.valueOf();
};
