...
module.exports = function(self) {
  ...
  */
  self.serverInfo = function(cb) {
    self._doRequest('serverInfo', [], self._getServerActionCallback(cb, [
      ...
      //using a callback that gets called before control returned to user.
      //this will add attributes that we need.
      info.currentMap = self.convertMapToHumanReadable(info.currentMapKey);
      info.currentGameMode = self.convertModeToHumanReadable(info.currentGameModeKey);
    }));
  };
  ...
  */
  ...
  self.punkBuster.pb_sv_command = function(cmd, cb) {
    self._doRequest('punkBuster.pb_sv_command', [cmd], self._getServerActionCallback(cb));
  };
  ...
  self.banList.addBan = function(idType, id, duration, numSeconds, reason, cb) {
    var fn = self.banList.add[idType];
    if(typeof fn === 'undefined') return cb('InvalidBanIdType');
    fn = fn(id);
    if(duration === 'perm') {
      if(typeof reason === 'undefined') fn.perm(cb);
      else fn.perm(reason, cb);
    } else if(duration === 'seconds') {
      if(typeof reason === 'undefined') fn.seconds(numSeconds, cb);
      else fn.seconds(numSeconds, reason, cb);
    } else return cb('InvalidBanDuration');
  };
  ...
  self.banList.add.name = function(soldierName) {
    return self.banList._genTimeoutFn('name', soldierName); 
  };
  ...
  self.banList.removeBan = function(idType, id, cb) {
    var fn = self.banList.remove[idType];
    if(typeof fn === 'undefined') return cb('InvalidBanIdType');
    return fn(id, cb);
  };
  ...
  self.banList.remove.name = function(soldierName, cb) {
    self._doRequest('banList.remove', ['name', soldierName], self._getServerActionCallback(cb)); 
  };
