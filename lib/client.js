...
var Client = module.exports = function(ip, port, password) {
  ...
  self._client.on('disconnect', function() {
   ...
   self.login.hashedLogin(password);
  }
}
...
Client.prototype.queueRequests = function() {
...
Client.prototype._doRequest = function(command, words, cb) {
  ...
  if(this.queueRequests()) {
    this._queue.add(bufObj.buffer, function(buf) {
      self._client.write(buf);
    });
  } else {
    self._client.write(bufObj.buffer);
  }
};
      ...
      if(!mvType) {
        ...
        if(cast && typeof cast === 'object') {
          ...
          var casterFn = cast[property]
            , casterInstance = new casterFn(word);
