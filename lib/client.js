  ...
  self.connect = function() {
    ...
    self._handleData = function(data) {
      ...
      if(result.isResponse) {
        ...
        if(typeof cmdObj.timeout !== 'undefined') clearTimeout(cmdObj.timeout);
        delete self._requests[result.sequence];

        var reqs = "TOOMANY cb's";
        if(Object.keys(self._requests).length <= 5) reqs = Object.keys(self._requests).join(",");
        console.log(self.ip, self.port, "received seq", result.sequence, "words", result.words, "reqs", reqs)
...
Client.prototype._wordsToBuffer = function(isFromServer, isResponse, words) {
  var i = 0
    ...
    console.log(this.ip, this.port, "creating seq", ret.sequence, "words", words)
...
Client.prototype._sendCommand = function(words, cb, tries) {
  var self = this;

  if(typeof tries == 'undefined') tries = 0;

  var bufObj = self._wordsToBuffer(false, false, words);
  var cmdObj = {tries: tries, words: words, sequence: bufObj.sequence}
  if(typeof cb === 'function') {
    cmdObj.callback = cb;
  }

  self._requests[bufObj.sequence] = cmdObj;
  ...
  if(words[0].indexOf('login.') !== 0 && self._queueRequests()) {
    ...
    //Need to do a timeout so that if the server does not respond, we can re-try.
    if(words[0] !== 'quit') self._requests[bufObj.sequence].timeout = setTimeout(timeoutClosure(bufObj.sequence), 1000);

    self._sendBuffer(bufObj.buffer);
  }

  function timeoutClosure(sequence) {
    return function() {
      var cmdObj = self._requests[sequence];
      if(!cmdObj) {
        console.log('null cmdObj', sequence)
        return;
      }
      console.log('cmdObj', cmdObj, sequence, self._requests)
      delete self._requests[sequence];
      if(cmdObj.tries < 3) self._sendCommand(words, cb, cmdObj.tries+1);
      else cb('RequestTimedOut');
    };
  }
};
