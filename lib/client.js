...
Client.prototype._getServerActionCallback = function(cb, casting) {
  ...
  return function(err, words) {
    ...
    if(!Array.isArray(casting)) casting = [casting];
    ...
    for(var castItr = 0, wordItr = 0; wordItr < words.length && castItr < casting.length; ++wordItr) {
      ...
      if(!mvType) {
          ...
          if(casterFn.name) {
            ...
            if(casterInstance instanceof MultiValueType) {
              ...
              if(mvType.isFinished()) {
                if(property !== null) info[property] = mvType.valueOf();
                else info = mvType.valueOf();
                ++castItr;
                mvType = null;
              }
            } else {
        ...
        if(!mvType) {
          ...
          if(property !== null) info[property] = castedWord;
          else info = castedWord;
          ++castItr;
        }
      } else {
